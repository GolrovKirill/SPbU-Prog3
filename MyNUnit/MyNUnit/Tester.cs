...
public static class Tester
{
    ...
    public static async Task<List<MyTestResult>> RunTestsAsync(string path)
    {
        ...
        {
            var testClasses = assembly.GetTypes().Where(t => t.GetMethods().Any(m => m.GetCustomAttributes(typeof(MyAttribute), false).Length != 0));
    ...
    /// <summary>
    /// Method for executing tests and displaying results.
    /// </summary>
    ...
    private static async Task<List<MyTestResult>> RunTestClass(Type testClass)
    {
        ...
        {
            var testAttr = (MyAttribute)method.GetCustomAttributes(typeof(MyAttribute), false).First();
            ...
            try
            {
                ...
                {
                    if (testAttr.Expected == null || ex.InnerException!.GetType() != testAttr.Expected) throw;
